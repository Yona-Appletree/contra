...
- `src/program.ts` — the evening. Every dance of `@caller/contra`'s
  ...
  a beat back as "which dance, which time through, or lining up".
...
- `src/routes/hall.tsx` — the page. One clock: a silent one on
  `performance.now` until somebody presses play, and `@caller/music`'s
  player clock — a linear function of `AudioContext.currentTime` — from then
  on (plan AC4). Nothing else reads a timer.
...
selector, the audio clock, and the two front-page goldens),
...
and the per-figure strips). The strips are written to `e2e/strips/` on every
