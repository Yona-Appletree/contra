...
| `star`                       | 8     | `STAR RIGHT`                   | `hand` `"R"`, `places` 4 (quarters), `amount` 1 (a fraction of the whole star; multiplies `places`, M8), `holdDrop` 3, `stackPx` 1.2, `hold` `"wrist"` — **data**                                                                                 |
...
| `single-file-promenade`      | 8     | `SINGLE FILE PROMENADE`        | `direction` `"clockwise"`, `amount` 0.25 (of the ring, whose size is read off the cast since M9) — **data**, no coded twin                                                                                                                        |
...
| `turn-contra-corners`        | 16    | `TURN CONTRA CORNERS`          | `holdDrop` 2, `axis` `"across"` (or `"along"`, which swaps the two corners, M9); five turns, 2 + 4 + 2 + 4 + 4 — **data**, no coded twin                                                                                                          |
...
| `promenade`                  | 8     | `PROMENADE AROUND THE SET`     | `pairs` `"neighbors"`, `direction` `"counterclockwise"`, `places` 2, `spacing` 14, `handDrop` 3 — **data**, no coded twin, `actors: "pairs"`, the `unit` kind (M8)                                                                                |
| `balance-wave-of-four`       | 4     | `BALANCE THE WAVE OF FOUR`     | `centre` `"robin"`, `hand` `"R"`, `rock` 4 px, `spacing` 14, `closeBeats` 1, `holdDrop` 2 — **data**, no coded twin, the wave **across** the set (M8)                                                                                             |
| `diamond`                    | 8     | `FORM DIAMONDS`                | `actives` `[["1L","1R"]]`, `outPx` 10, `apartPx` 14, `leadRole` `"lark"` — **data**, no coded twin, `ends: { target: { shape: "diamond" } }` (M9)                                                                                                 |
